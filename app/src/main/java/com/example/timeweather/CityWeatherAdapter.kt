    ...
    RecyclerView.Adapter<CityWeatherAdapter.CityWeatherViewHolder>() {
        class CityWeatherViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
            ...
            fun populaAdapter(city : City){
                textViewNome.text = city.name
                textViewClima.text = city.clima
                val decimalFormat = DecimalFormat("##.#")
                textviewTemperatura.text = decimalFormat.format(city.temperatura)
                textviewTemperaturaMinima.text = decimalFormat.format(city.temp_min)
                textviewTemperaturaMaxima.text = decimalFormat.format(city.temp_max)
            }
